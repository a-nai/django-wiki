        ...
        if not path is None:
            ...
            if urlpath.article:
                ...
                try:
                 ...
                 user=articleread1.objects.create(current_revision=article.current_revision, read='1',user_id=request.user.id,article_id=article.id, paid='0',readed=datetime.datetime.now(),last=datetime.datetime.now())    
                 user.save()
                 import types
